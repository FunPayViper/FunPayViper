...
from Utils import config_loader as cfg_loader, exceptions as excs, viper_tools
...
def init_uploader(viper: Viper):
    ...
    def upload_products_file(m: types.Message):
        ...
        try:
            products_count = viper_tools.count_products(f"storage/products/{utils.escape(m.document.file_name)}")
        except:
    ...
    def upload_main_config(m: types.Message):
        ...
        viper.save_config(new_config, "configs/_main.cfg")
        logger.info(f"Пользователь $MAGENTA@{m.from_user.username} (id: {m.from_user.id})$RESET "
    ...
    def upload_auto_response_config(m: types.Message):
        ...
        viper.RAW_AR_CFG, viper.AR_CFG = raw_new_config, new_config
    ...
    def upload_auto_delivery_config(m: types.Message):
        ...
        viper.AD_CFG = new_config
        viper.save_config(viper.AD_CFG, "configs/auto_delivery.cfg")
    ...
    def send_funpay_image(m: types.Message):
        ...
        try:
            ...
            file = tg.bot.download_file(file_info.file_path)
            image_id = viper.account.upload_image(file, type_="chat")
            result = viper.account.send_message(chat_id, None, username, image_id)
            if not result:
    ...
    def upload_image(m: types.Message, type_: Literal["chat", "offer"] = "chat"):
        ...
        try:
            file_info = tg.bot.get_file(photo.file_id)
            file = tg.bot.download_file(file_info.file_path)
            image_id = viper.account.upload_image(file, type_=type_)
        except:
