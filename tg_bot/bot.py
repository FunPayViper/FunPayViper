...
from Utils import viper_tools, updater
...
class TGBot:
    def __init__(self, viper: Viper):
        self.viper = viper
        self.bot = telebot.TeleBot(self.viper.MAIN_CFG["Telegram"]["token"], parse_mode="HTML",
    ...
    def reg_admin(self, m: Message):
        ...
        if m.chat.type != "private" or (self.attempts.get(m.from_user.id, 0) >= 5) or m.text is None:
            return
        if not self.viper.block_tg_login and \
                viper_tools.check_password(m.text, self.viper.MAIN_CFG["Telegram"]["secretKeyHash"]):
    ...
    def send_profile(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, utils.generate_profile_text(self.viper),
    ...
    def change_cookie(self, m: Message):
        ...
        new_account = Account(golden_key, self.viper.account.user_agent, proxy=self.viper.proxy,
                              locale=self.viper.account.locale)
        try:
            new_account.get()
        except:
        ...
        one_acc = False
        if new_account.id == self.viper.account.id or self.viper.account.id is None:
            one_acc = True
            self.viper.account.golden_key = golden_key
            try:
                self.viper.account.get()
            except:
                logger.warning("Произошла ошибка")  # locale
                logger.debug("TRACEBACK", exc_info=True)
                self.bot.send_message(m.chat.id, _("cookie_error"))
            ...
            accs = f" (<a href='https://funpay.com/users/{self.viper.account.id}/'>" \
                   f"{self.viper.account.username}</a> ➔ <a href='https://funpay.com/users/{new_account.id}/'>" \
                   f"{new_account.username}</a>)"

        self.viper.MAIN_CFG.set("FunPay", "golden_key", golden_key)
        self.viper.save_config(self.viper.MAIN_CFG, "configs/_main.cfg")
    ...
    def update_profile(self, c: CallbackQuery):
        ...
        try:
            self.viper.account.get()
            self.viper.balance = self.viper.get_balance()
        except:
        ...
        self.bot.edit_message_text(utils.generate_profile_text(self.viper), c.message.chat.id,
    ...
    def manual_delivery_text(self, m: Message):
        ...
        self.viper.delivery_tests[key] = lot_name
    ...
    def ban(self, m: Message):
        ...
        if nickname in self.viper.blacklist:
            self.bot.send_message(m.chat.id, _("already_blacklisted", nickname))
            return

        self.viper.blacklist.append(nickname)
        viper_tools.cache_blacklist(self.viper.blacklist)
    ...
    def unban(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        nickname = m.text.strip()
        if nickname not in self.viper.blacklist:
            self.bot.send_message(m.chat.id, _("not_blacklisted", nickname))
            return
        self.viper.blacklist.remove(nickname)
        viper_tools.cache_blacklist(self.viper.blacklist)
    ...
    def send_ban_list(self, m: Message):
        ...
        Отправляет ЧС.
        """
        if not self.viper.blacklist:
            self.bot.send_message(m.chat.id, _("blacklist_empty"))
            return
        blacklist = ", ".join(f"<code>{i}</code>" for i in sorted(self.viper.blacklist, key=lambda x: x.lower()))
    ...
    def act_edit_watermark(self, m: Message):
        ...
        """
        watermark = self.viper.MAIN_CFG["Other"]["watermark"]
    ...
    def edit_watermark(self, m: Message):
        ...
        preview = f"<a href=\"https://sfunpay.com/s/chat/zb/wl/zbwl4vwc8cc1wsftqnx5.jpg\">⁢</a>" if not \
            any([i.lower() in watermark.lower() for i in ("🐍", "FPV", "𝙁𝙋𝙑", "𝙑𝙞𝙥𝙚𝙧", "Viper", "Вайпер")]) else \
            f"<a href=\"https://sfunpay.com/s/chat/kd/8i/kd8isyquw660kcueck3g.jpg\">⁢</a>"
        self.viper.MAIN_CFG["Other"]["watermark"] = watermark
        self.viper.save_config(self.viper.MAIN_CFG, "configs/_main.cfg")
        if watermark:
            ...
            try:
                with open("logs/log.log", "r", encoding="utf-8") as f:
                    self.bot.send_document(m.chat.id, f,
                                           caption=f'{_("gs_old_msg_mode").replace("{} ", "") if self.viper.old_mode_enabled else ""}')
                    f.seek(0)
                    file_content = f.read()
    ...
    def about(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("about", self.viper.VERSION))

    def check_updates(self, m: Message):
        curr_tag = f"v{self.viper.VERSION}"
    ...
    def send_system_info(self, m: Message):
        ...
        uptime = current_time - self.viper.start_time
        ...
        self.bot.send_message(m.chat.id, _("sys_info", cpu_usage, psutil.Process().cpu_percent(),
                                           ...
                                           viper_tools.time_to_str(uptime), m.chat.id))

    def restart_viper(self, m: Message):
        """
        Перезапускает вайпер.
        """
        self.bot.send_message(m.chat.id, _("restarting"))
        viper_tools.restart_program()

    def ask_power_off(self, m: Message):
        """
        Просит подтверждение на отключение FPV.
        """
        self.bot.send_message(m.chat.id, _("power_off_0"), reply_markup=kb.power_off(self.viper.instance_id, 0))
    ...
    def power_off(self, c: CallbackQuery):
        ...
        if instance_id != self.viper.instance_id:
    ...
    def send_funpay_message(self, message: Message):
        ...
        result = self.viper.send_message(node_id, response_text, username, watermark=False)
        if result:
    ...
    def edit_greetings_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.viper.MAIN_CFG["Greetings"]["greetingsText"] = m.text
    ...
    def edit_greetings_cooldown(self, m: Message):
        ...
        try:
            ...
            return
        self.viper.MAIN_CFG["Greetings"]["greetingsCooldown"] = str(cooldown)
    ...
    def edit_order_confirm_reply_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.viper.MAIN_CFG["OrderConfirm"]["replyText"] = m.text
    ...
    def edit_review_reply_text(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.viper.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"] = m.text
    ...
    def extend_new_message_notification(self, c: CallbackQuery):
        ...
        try:
            chat = self.viper.account.get_chat(int(chat_id))
        except:
            self.bot.answer_callback_query(c.id)
        ...
        for i in messages:
            if i.author_id == last_message_author_id and i.by_bot == last_by_bot and i.badge == last_badge and \
                ...
                author = ""
            elif i.author_id == self.viper.account.id:
                ...
                elif i.author in self.viper.blacklist:
                    author = f"<i><b>🚷 {i.author}: </b></i>"
                elif i.by_bot:
                    author = f"<i><b>🐦 {i.author}: </b></i>"
                elif i.by_vertex:
            ...
            msg_text = f"<code>{utils.escape(i.text)}</code>" if i.text else \
                ...
                f"{self.viper.show_image_name and not (i.author_id == self.viper.account.id and i.by_bot) and i.image_name or _('photo')}</a>"
    ...
    def refund(self, c: CallbackQuery):
        ...
        while attempts:
            try:
                self.viper.account.refund(order_id)
                break
            except:
                if not new_msg:
    ...
    def switch_param(self, c: CallbackQuery):
        ...
        if section == "FunPay" and option == "oldMsgGetMode":
            self.viper.switch_msg_get_mode()
        else:
            self.viper.MAIN_CFG[section][option] = str(int(not int(self.viper.MAIN_CFG[section][option])))
            self.viper.save_config(self.viper.MAIN_CFG, "configs/_main.cfg")
        ...
        if section == "Telegram":
            self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                               reply_markup=kb.authorized_users(self.viper, offset=int(split[3])))
        else:
            self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                               reply_markup=sections[section](self.viper))
        logger.info(_("log_param_changed", c.from_user.username, c.from_user.id, option, section,
                      self.viper.MAIN_CFG[section][option]))
        self.bot.answer_callback_query(c.id)
    ...
    def switch_chat_notification(self, c: CallbackQuery):
        ...
        self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                           reply_markup=keyboard(self.viper, c.message.chat.id))
        self.bot.answer_callback_query(c.id)
    ...
    def open_settings_section(self, c: CallbackQuery):
        ...
        sections = {
            "lang": (_("desc_lang"), kb.language_settings, [self.viper]),
            "main": (_("desc_gs"), kb.main_settings, [self.viper]),
            "tg": (_("desc_ns", c.message.chat.id), kb.notifications_settings, [self.viper, c.message.chat.id]),
            "bl": (_("desc_bl"), kb.blacklist_settings, [self.viper]),
            "ar": (_("desc_ar"), skb.AR_SETTINGS, []),
            "ad": (_("desc_ad"), skb.AD_SETTINGS, []),
            "mv": (_("desc_mv"), kb.new_message_view_settings, [self.viper]),
            "rr": (_("desc_or"), kb.review_reply_settings, [self.viper]),
            "gr": (_("desc_gr", utils.escape(self.viper.MAIN_CFG['Greetings']['greetingsText'])),
                   kb.greeting_settings, [self.viper]),
            "oc": (_("desc_oc", utils.escape(self.viper.MAIN_CFG['OrderConfirm']['replyText'])),
                   kb.order_confirm_reply_settings, [self.viper])
        }
    ...
    def send_announcements_kb(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("desc_an"), reply_markup=kb.announcements_settings(self.viper, m.chat.id))

    def send_review_reply_text(self, c: CallbackQuery):
        stars = int(c.data.split(":")[1])
        text = self.viper.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"]
        keyboard = K() \
            .row(B(_("gl_back"), callback_data=f"{CBT.CATEGORY}:rr"),
                 B(_("gl_edit"), callback_data=f"{CBT.EDIT_REVIEW_REPLY_TEXT}:{stars}"))
        if not text:
            ...
            self.bot.send_message(c.message.chat.id, _("review_reply_text", "⭐" * stars,
                                                       self.viper.MAIN_CFG['ReviewReply'][f'star{stars}ReplyText']),
    ...
    def switch_lang(self, c: CallbackQuery):
        ...
        if localizer.current_language == "en":
            ...
            self.bot.answer_callback_query(c.id, "Переклад складено за допомогою ChatGPT.\n"
                                                 "Повідомте @beedge, якщо знайдете помилки.", show_alert=True)
    ...
    def __register_handlers(self):
        ...
        self.msg_handler(self.restart_viper, commands=["restart"])
    ...
    def edit_bot(self):
        ...
        sh_text = "🛠️ github.com/FunPayViper/FunPayViper 💰 @beedge_donate 👨‍💻 @beedge 🧩 @fpv_plugins 🔄 @fpv_updates 💬 @funpay_viper"
        ...
        for i in [None, *localizer.languages.keys()]:
            ...
            text = _("adv_description", self.viper.VERSION, language=i)
            if res != text:
