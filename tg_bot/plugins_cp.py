...
def init_plugins_cp(viper: Viper, *args):
    tg = viper.telegram
    bot = tg.bot
    ...
    def open_plugins_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_pl"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.plugins_list(viper, offset))
    ...
    def open_edit_plugin_cp(c: CallbackQuery):
        ...
        plugin_data = viper.plugins[uuid]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
        keyboard = keyboards.edit_plugin(viper, uuid, offset)
    ...
    def toggle_plugin(c: CallbackQuery):
        ...
        viper.toggle_plugin(uuid)
        c.data = f"{CBT.EDIT_PLUGIN}:{uuid}:{offset}"
        logger.info(_("log_pl_activated" if viper.plugins[uuid].enabled else "log_pl_deactivated",
                      c.from_user.username, c.from_user.id, viper.plugins[uuid].name))
    ...
    def ask_delete_plugin(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                      reply_markup=keyboards.edit_plugin(viper, uuid, offset, True))
        bot.answer_callback_query(c.id)
    ...
    def cancel_delete_plugin(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                      reply_markup=keyboards.edit_plugin(viper, uuid, offset))
        bot.answer_callback_query(c.id)
    ...
    def delete_plugin(c: CallbackQuery):
        ...
        if not os.path.exists(viper.plugins[uuid].path):
            bot.answer_callback_query(c.id, _("pl_file_not_found_err", utils.escape(viper.plugins[uuid].path)),
                                      show_alert=True)
            return

        if viper.plugins[uuid].delete_handler:
            try:
                viper.plugins[uuid].delete_handler(viper, c)
            except:
                logger.error(_("log_pl_delete_handler_err", viper.plugins[uuid].name))
                logger.debug("TRACEBACK", exc_info=True)

        os.remove(viper.plugins[uuid].path)
        logger.info(_("log_pl_deleted", c.from_user.username, c.from_user.id, viper.plugins[uuid].name))
        viper.plugins.pop(uuid)
