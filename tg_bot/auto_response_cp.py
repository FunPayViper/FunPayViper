...
def init_auto_response_cp(viper: Viper, *args):
    tg = viper.telegram
    bot = tg.bot
    ...
    def check_command_exists(command_index: int, message_obj: Message, reply_mode: bool = True) -> bool:
        ...
        """
        if command_index > len(viper.RAW_AR_CFG.sections()) - 1:
    ...
    def open_commands_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_ar_list"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.commands_list(viper, offset))
    ...
    def add_command(m: Message):
        ...
        for cmd in commands:
            if commands.count(cmd) > 1:
                ...
                return
            if cmd in viper.AR_CFG.sections():
                bot.reply_to(m, _("ar_cmd_already_exists_err", utils.escape(cmd)), reply_markup=error_keyboard)
                return

        viper.RAW_AR_CFG.add_section(raw_command)
        viper.RAW_AR_CFG.set(raw_command, "response", "Данной команде необходимо настроить текст ответа :(")
        viper.RAW_AR_CFG.set(raw_command, "telegramNotification", "0")

        for cmd in commands:
            viper.AR_CFG.add_section(cmd)
            viper.AR_CFG.set(cmd, "response", "Данной команде необходимо настроить текст ответа :(")
            viper.AR_CFG.set(cmd, "telegramNotification", "0")

        viper.save_config(viper.RAW_AR_CFG, "configs/auto_response.cfg")

        command_index = len(viper.RAW_AR_CFG.sections()) - 1
    ...
    def open_edit_command_cp(c: CallbackQuery):
        ...
        keyboard = keyboards.edit_command(viper, command_index, offset)

        command = viper.RAW_AR_CFG.sections()[command_index]
        command_obj = viper.RAW_AR_CFG[command]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
    ...
    def edit_command_response(m: Message):
        ...
        viper.RAW_AR_CFG.set(command, "response", response_text)
        for cmd in commands:
            viper.AR_CFG.set(cmd, "response", response_text)
        viper.save_config(viper.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def edit_command_notification(m: Message):
        ...
        command = viper.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        viper.RAW_AR_CFG.set(command, "notificationText", notification_text)

        for cmd in commands:
            viper.AR_CFG.set(cmd, "notificationText", notification_text)
        viper.save_config(viper.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def switch_notification(c: CallbackQuery):
        ...
        if command_obj.get("telegramNotification") in [None, "0"]:
            ...
            value = "0"
        viper.RAW_AR_CFG.set(command, "telegramNotification", value)
        for cmd in commands:
            viper.AR_CFG.set(cmd, "telegramNotification", value)
        viper.save_config(viper.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def del_command(c: CallbackQuery):
        ...
        command = viper.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        viper.RAW_AR_CFG.remove_section(command)
        for cmd in commands:
            viper.AR_CFG.remove_section(cmd)
        viper.save_config(viper.RAW_AR_CFG, "configs/auto_response.cfg")
