...
def language_settings(c: Viper) -> K:
...
def new_message_view_settings(c: Viper) -> K:
    """
...
def order_confirm_reply_settings(c: Viper):
    """
...
def authorized_users(c: Viper, offset: int):
    """
...
def authorized_user_settings(c: Viper, user_id: int, offset: int, user_link: bool):
    """
...
def proxy(c: Viper, offset: int, proxies: dict[str, bool]):
    """
...
def review_reply_settings(c: Viper):
    """
...
def notifications_settings(c: Viper, chat_id: int) -> K:
    """
...
def announcements_settings(c: Viper, chat_id: int):
    """
...
def blacklist_settings(c: Viper) -> K:
    """
...
def commands_list(c: Viper, offset: int) -> K:
    """
...
def edit_command(c: Viper, command_index: int, offset: int) -> K:
    """
...
def products_files_list(offset: int) -> K:
    ...
    for index, name in enumerate(files):
        amount = Utils.viper_tools.count_products(f"storage/products/{name}")
...
def lots_list(viper: Viper, offset: int) -> K:
    """
    ...
    """
    keyboard = K()
    lots = viper.AD_CFG.sections()[offset: offset + MENU_CFG.AD_BTNS_AMOUNT]
    ...
    keyboard = add_navigation_buttons(keyboard, offset, MENU_CFG.AD_BTNS_AMOUNT, len(lots),
                                      len(viper.AD_CFG.sections()), CBT.AD_LOTS_LIST)
...
def funpay_lots_list(c: Viper, offset: int):
    """
...
def edit_lot(c: Viper, lot_number: int, offset: int) -> K:
    """
    Генерирует клавиатуру изменения лота (CBT.EDIT_AD_LOT:<lot_num>:<offset>).

    :param c: экземпляр вайпера.
...
def templates_list(c: Viper, offset: int) -> K:
    """
...
def edit_template(c: Viper, template_index: int, offset: int) -> K:
    """
...
def templates_list_ans_mode(c: Viper, offset: int, node_id: int, username: str, prev_page: int,
...
def plugins_list(c: Viper, offset: int):
    """
...
def edit_plugin(c: Viper, uuid: str, offset: int, ask_to_delete: bool = False):
    """
...
def LINKS_KB(language: None | str = None) -> K:
    return K().add(B(_("lnk_github", language=language),
                     url="https://github.com/FunPayViper/FunPayViper")) \
        .add(B(_("lnk_updates", language=language), url="https://t.me/fpv_updates")) \
        .add(B(_("mm_plugins", language=language), url="https://t.me/fpv_plugins")) \
        .add(B(_("lnk_chat", language=language), url="https://t.me/funpay_viper"))
