...
# INIT MESSAGE
def save_init_chats_handler(c: Viper, e: InitialChatEvent):
    """
    ...
    if c.MAIN_CFG["Greetings"].getboolean("sendGreetings") and e.chat.id not in c.old_users:
        ...
        viper_tools.cache_old_users(c.old_users)


# NEW MESSAGE / LAST CHAT MESSAGE CHANGED
def old_log_msg_handler(c: Viper, e: LastChatMessageChangedEvent):
    """
...
def log_msg_handler(c: Viper, e: NewMessageEvent):
...
def greetings_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    text = viper_tools.format_msg_text(c.MAIN_CFG["Greetings"]["greetingsText"], obj)
    Thread(target=c.send_message, args=(chat_id, text, chat_name), daemon=True).start()


def add_old_user_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_response_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    response_text = viper_tools.format_msg_text(c.AR_CFG[command]["response"], obj)
    Thread(target=c.send_message, args=(chat_id, response_text, chat_name), daemon=True).start()


def old_send_new_msg_notification_handler(c: Viper, e: LastChatMessageChangedEvent):
...
def send_new_msg_notification_handler(c: Viper, e: NewMessageEvent) -> None:
    """
...
def send_review_notification(c: Viper, order: Order, chat_id: int, reply_text: str | None):
    if not c.telegram:
        return
...
def process_review_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
        obj = e.message
    ...
    def send_reply():
        ...
        if c.MAIN_CFG["ReviewReply"].getboolean(toggle) and c.MAIN_CFG["ReviewReply"].get(text):
            try:
                ...
                reply_text = viper_tools.format_order_text(c.MAIN_CFG["ReviewReply"].get(text), order)
...
def send_command_notification_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    if not c.AR_CFG[command].get("notificationText"):
        ...
        text = viper_tools.format_msg_text(c.AR_CFG[command]["notificationText"], obj)
...
def test_auto_delivery_handler(c: Viper, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_categories_raised_notification_handler(c: Viper, cat: types.Category, error_text: str = "") -> None:
    """
...
def get_lot_config_by_name(c: Viper, name: str) -> configparser.SectionProxy | None:
    """
    Ищет секцию лота в конфиге автовыдачи.

    :param c: объект вайпера.
...
def check_products_amount(config_obj: configparser.SectionProxy) -> int:
    ...
    if not file_name:
        return 1
    return viper_tools.count_products(f"storage/products/{file_name}")


def update_current_lots_handler(c: Viper, e: OrdersListChangedEvent):
...
def update_profile_lots_handler(c: Viper, e: OrdersListChangedEvent):
...
def log_new_order_handler(c: Viper, e: NewOrderEvent, *args):
    """
...
def setup_event_attributes_handler(c: Viper, e: NewOrderEvent, *args):
...
def send_new_order_notification_handler(c: Viper, e: NewOrderEvent, *args):
    """
...
def deliver_goods(c: Viper, e: NewOrderEvent, *args):
    chat_id = c.account.get_chat_by_name(e.order.buyer_username).id
    cfg_obj = getattr(e, "config_section_obj")
    delivery_text = viper_tools.format_order_text(cfg_obj["response"], e.order)
    ...
    try:
        if file_name := cfg_obj.get("productsFileName"):
            ...
            products, goods_left = viper_tools.get_products(f"storage/products/{file_name}", amount)
    ...
    if not result:
        ...
        if file_name and products:
            viper_tools.add_products(f"storage/products/{file_name}", products, at_zero_position=True)
    else:
...
def deliver_product_handler(c: Viper, e: NewOrderEvent, *args) -> None:
    """
...
def send_delivery_notification_handler(c: Viper, e: NewOrderEvent):
    """
...
def update_lot_state(viper: Viper, lot: types.LotShortcut, task: int) -> bool:
    """
    Обновляет состояние лота

    :param viper: объект Вайпера.
    ...
    while attempts:
        try:
            lot_fields = viper.account.get_lot_fields(lot.id)
            if task == 1:
                lot_fields.active = True
                viper.account.save_lot(lot_fields)
...
def update_lots_states(viper: Viper, event: NewOrderEvent):
    if not any([viper.autorestore_enabled, viper.autodisable_enabled]):
        return
    if viper.curr_profile_last_tag != event.runner_tag or viper.last_state_change_tag == event.runner_tag:
        return

    lots = viper.curr_profile.get_sorted_lots(1)

    deactivated = []
    restored = []
    for lot in viper.profile.get_lots():
        # -1 - деактивировать
        # 0 - ничего не делать
        # 1 - восстановить
        current_task = 0
        config_obj = get_lot_config_by_name(viper, lot.description)
        ...
        # Если же лот активен
            ...
            if config_obj:
                ...
                if all((not products_count, viper.MAIN_CFG["FunPay"].getboolean("autoDisable"),
                        config_obj.get("disableAutoDisable") in ["0", None])):
                    current_task = -1

        if current_task:
            result = update_lot_state(viper, lot, current_task)
            if result:
                if current_task == -1:
...
<code>{lots}</code>"""
        Thread(target=viper.telegram.send_notification, args=(text,),
...
<code>{lots}</code>"""
    ...
    viper.last_state_change_tag = event.runner_tag


def update_lots_state_handler(viper: Viper, event: NewOrderEvent, *args):
    Thread(target=update_lots_states, args=(viper, event), daemon=True).start()


# BIND_TO_ORDER_STATUS_CHANGED
def send_thank_u_message_handler(c: Viper, e: OrderStatusChangedEvent):
    """
    ...
    text = viper_tools.format_order_text(c.MAIN_CFG["OrderConfirm"]["replyText"], e.order)
...
def send_order_confirmed_notification_handler(viper: Viper, event: OrderStatusChangedEvent):
    """
    ...
    chat = viper.account.get_chat_by_name(event.order.buyer_username, True)
    Thread(target=viper.telegram.send_notification,  # locale
           args=(
...
def send_bot_started_notification_handler(c: Viper, *args):
    """
