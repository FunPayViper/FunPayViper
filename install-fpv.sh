...
echo -e "\n\n${RED} * GitHub ${CYAN}github.com/FunPayViper/FunPayViper${RESET}"
echo -e "${RED} * Telegram ${CYAN}t.me/funpay_Viper${RESET}"
echo -e "\n\n\n"


echo -ne "${CYAN}Введите имя пользователя, от имени которого будет запускаться бот (например, 'fpv' или 'viper'): ${RESET}"
while true; do
  read username
...
clear
echo -e "$start_process_line\nУстанавливаю FunPayViper...\n$end_process_line"


#13
if ! sudo mkdir /home/$username/fpv-install ; then
  echo -e "${start_process_line}\nПроизошла ошибка при создании директории для установки. (13/${commands})\n${end_process_line}"
  exit 2
fi

gh_repo="FunPayViper/FunPayViper"
...
#15
if ! sudo unzip /home/$username/fpv-install/fpv.zip -d /home/$username/fpv-install ; then
  echo -e "${start_process_line}\nПроизошла ошибка при распаковке архива. (15/${commands})\n${end_process_line}"
  exit 2
fi

#16
if ! sudo mkdir /home/$username/FunPayViper ; then
  echo -e "${start_process_line}\nПроизошла ошибка при создании директории для бота. (16/${commands})\n${end_process_line}"
  exit 2
fi

#17
if ! sudo mv /home/$username/fpv-install/*/* /home/$username/FunPayViper/ ; then
...
#19
if ! sudo chown -hR $username:$username /home/$username/FunPayViper ; then
  echo -e "${start_process_line}\nПроизошла ошибка при изменении владельца файлов. (19/${commands})\n${end_process_line}"
  exit 2
fi

#20
if ! sudo -u $username /home/$username/pyvenv/bin/pip install -U -r /home/$username/FunPayViper/requirements.txt ; then
...
#21
if ! sudo ln -sf /home/$username/FunPayViper/FunPayViper@.service /etc/systemd/system/FunPayViper@.service ; then
...
clear
echo -e $logo
echo -e '\n\n\e[1;91m * GitHub \e[1;96mgithub.com/FunPayViper/FunPayViper\e[0m'
echo -e '\e[1;91m * Telegram \e[1;96mt.me/funpay_Viper\e[0m'
...
sudo -u $username LANG=en_US.utf8 /home/$username/pyvenv/bin/python /home/$username/FunPayViper/main.py <&1
sudo systemctl start FunPayViper@$username.service
...
echo -e "\n\e[1;92mДля остановки FPV используй команду \e[93msudo systemctl stop FunPayViper@${username}\e[1;92m"
echo -e "Для запуска FPV используй команду \e[93msudo systemctl start FunPayViper@${username}\e[1;92m"
echo -e "Для перезапуска FPV используй команду \e[93msudo systemctl restart FunPayViper@${username}\e[1;92m"
echo -e "Для просмотра логов используй команду \e[93msudo systemctl status FunPayViper@${username} -n100\e[1;92m"
echo -e "Для добавления FPV в автозагрузку используй команду \e[93msudo systemctl enable FunPayViper@${username}\e[1;92m"
