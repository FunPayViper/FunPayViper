...
logger = getLogger("FPV.announcements")
localizer = Localizer()
...
def get_announcement(ignore_last_tag: bool = False) -> dict | None:
    ...
    try:
        ...
        content = json.loads(response.json().get("files").get("fpv.json").get("content"))
...
def announcements_loop_iteration(crd: Viper, ignore_last_tag: bool = False):
    global LAST_TAG
...
def announcements_loop(crd: Viper):
    """
