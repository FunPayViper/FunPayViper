...
logger = getLogger("FPV.update_checker")
localizer = Localizer()
...
def get_tags(current_tag: str) -> list[str] | None:
    ...
    try:
        ...
        while not any([el.get("name") == current_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            response = requests.get(f"https://api.github.com/repos/FunPayViper/FunPayViper/tags?page={page}",
                                    headers=HEADERS)
...
def get_releases(from_tag: str) -> list[Release] | None:
    ...
    try:
        ...
        while not any([el.get("tag_name") == from_tag for el in json_response]):
            if page != 1:
                time.sleep(1)
            response = requests.get(f"https://api.github.com/repos/FunPayViper/FunPayViper/releases?page={page}",
                                    headers=HEADERS)
            if not response.status_code == 200 or not response.json():
                logger.debug(f"Update status code is {response.status_code}!")
                return None
