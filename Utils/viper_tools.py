...
logger = logging.getLogger("FPV.viper_tools")
localizer = Localizer()
...
def load_old_users(greetings_cooldown: float) -> dict[int, float]:
    ...
    try:
        ...
        return dict()
    # todo убрать позже, конвертация для старых версий вайпера
    if type(users) == list:
